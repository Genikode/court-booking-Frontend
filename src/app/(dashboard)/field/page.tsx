...
export default function FootballFieldManagement() {
  const [fields, setFields] = useState<Field[]>([
    { id: 1, name: 'Field 1', type: '5-a-side', status: 'Available', slots: ['7pm - 8pm'] },
    { id: 2, name: 'Field 2', type: '7-a-side', status: 'Occupied', slots: ['8pm - 9pm'] },
  ]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editId) {
      ...
      const newField: Field = { ...form, id: Date.now() };
      setFields((prev) => [...prev, newField]);
    }
    setForm({ name: '', type: '5-a-side', status: 'Available', slots: [] });
  };
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Football Field Management</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded shadow p-4 space-y-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
            ...
            <select
              value={form.status}
              onChange={(e) => setForm({ ...form, status: e.target.value as FieldForm['status'] })}
              className="border p-2 rounded w-full"
            >
            ...
            <tbody>
              {fields.map((field) => (
                <tr key={field.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4">{field.status}</td>
